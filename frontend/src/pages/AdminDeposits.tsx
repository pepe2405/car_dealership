...
import { fetchAllDeposits, updateDepositStatus, AdminDeposit, UpdateDepositData } from '../services/adminDepositsService';
...
const AdminDeposits = () => {
  ...
  const loadDeposits = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await fetchAllDeposits(token!);
      setDeposits(data);
    } catch (err) {
  ...
  const handleStatusUpdate = async (depositId: string, data: UpdateDepositData) => {
    try {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-7xl mx-auto px-4">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Управление на депозити</h1>
          <p className="text-gray-600">Администрирайте всички депозити в системата</p>
        </div>
        ...
        {deposits.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-8 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">Все още няма създадени депозити в системата.</p>
          </div>
        ) : (
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          <div className="flex space-x-2">
                            {deposit.status === 'pending' && (
                              <>
                                <button
                                  onClick={() => handleStatusUpdate(deposit._id, { status: 'approved' })}
                                ...
                                <button
                                  onClick={() => handleStatusUpdate(deposit._id, { status: 'rejected' })}
                                  disabled={updatingDeposit === deposit._id}
                            ...
                            {deposit.status === 'approved' && (
                              <button
                                onClick={() => handleStatusUpdate(deposit._id, { status: 'refunded' })}
                                disabled={updatingDeposit === deposit._id}
                              ...
                              >
                                {updatingDeposit === deposit._id ? 'Обновяване...' : 'Върни'}
                              </button>
                            )}
                          </div>
                        </div>
