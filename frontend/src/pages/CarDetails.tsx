...
const CarDetails = () => {
  ...
  const [depositStatus, setDepositStatus] =
    useState<DepositStatusResponse | null>(null);
  ...
  const handleTestDriveRequest = async () => {
    ...
    const token = authService.getToken();
    if (!token) {
      setTestDriveError("Трябва да сте влезли.");
      return;
    }
    if (!testDriveDate) {
      setTestDriveError("Изберете дата и час.");
      return;
    }
    try {
      const newReq = await createTestDriveRequest(
        { car: car!._id, date: testDriveDate, message: testDriveMsg },
        token,
      );
          ...
          <div className="mb-6 relative">
            {car && (
              <div className="absolute top-4 right-4 z-10">
                <button
                  onClick={() => handleFavorite(car._id)}
              ...
              {depositLoading ? (
                ...
                <div className="bg-gray-50 rounded-lg p-6 text-center">
                  ...
                  <p className="text-gray-600 mb-4">
                    Направете депозит, за да запазите този автомобил и да
                    покажете сериозност в покупката.
                  </p>
                  <button
            ...
            <>
              {myTestDrive ? (
                <div className="mt-8 flex flex-col items-center justify-center gap-2">
                  <div
                    className={`px-4 py-2 rounded-xl text-lg font-bold shadow ${myTestDrive.status === "pending" ? "bg-yellow-100 text-yellow-700" : myTestDrive.status === "approved" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                  >
                    Заявка за тест драйв:{" "}
                    {myTestDrive.status === "pending"
                      ? "Изчаква"
                      : myTestDrive.status === "approved"
                        ? "Одобрена"
                        : "Отказана"}
                  </div>
                  <div className="text-gray-700">
                    Дата: {new Date(myTestDrive.date).toLocaleString("bg")}
                  </div>
                  {myTestDrive.message && (
            ...
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
              <div className="bg-white rounded-2xl p-8 shadow-2xl w-full max-w-md relative">
                ...
                <textarea
                  className="input w-full mb-4"
                  value={testDriveMsg}
                  onChange={(e) => setTestDriveMsg(e.target.value)}
                />
                <button
