...
const AdminCarsView = () => {
  ...
  const filteredModels = selectedBrand
    ? uniqueModels.filter((model) =>
        cars.some(
          (car) => car.brand === selectedBrand && car.carModel === model,
        ),
      )
    : uniqueModels;
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-primary-100">
      <div className="w-full max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        ...
        <p className="text-lg text-primary-700 mb-8">
          Преглед на всички автомобили в системата (включително заети)
        </p>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 mb-8 animate-slide-up border border-primary-100">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-primary-700 flex items-center gap-2">
                <FaGasPump className="text-primary-400" />
                Гориво
              </label>
              <select
                name="fuelType"
          ...
          <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {filteredCars.length === 0 ? (
              ...
              filteredCars.map((car) => (
                <div
                  key={car._id}
                  className="card relative group hover:shadow-2xl transition-all duration-300 bg-white rounded-2xl overflow-hidden border border-primary-100 animate-fade-in"
                >
                  <div className="relative h-56 overflow-hidden rounded-t-2xl">
                    ...
                    <div className="absolute top-4 left-4">
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(car.status)}`}
                      >
                  ...
                  <div className="p-6">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Продавач: {car.seller?.name || "N/A"}
                    </p>
                    <div className="mt-4 flex items-center justify-between">
                      <span className="text-2xl font-bold text-primary-600">
                        {car.price.toLocaleString("bg-BG")} лв.
                      </span>
                      <Link
                        to={`/cars/${car._id}`}
                        className="btn-primary text-sm rounded-xl px-4 py-2 flex items-center gap-2"
                      >
