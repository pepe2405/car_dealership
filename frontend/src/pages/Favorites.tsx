    ...
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 to-primary-100 py-12">
      <div className="max-w-7xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-primary-800 mb-8 flex items-center gap-2">
          <FaHeart className="text-red-500" /> Любими коли
        </h1>
        {favorites.length === 0 ? (
          <div className="text-center text-primary-700 text-xl">
            Нямате добавени любими коли.
          </div>
        ) : (
          <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {favorites.map((car) => (
              <div
                key={car._id}
                className="card group hover:shadow-2xl transition-all duration-300 bg-white rounded-2xl overflow-hidden border border-primary-100 animate-fade-in relative"
              >
