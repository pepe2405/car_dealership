...
import {
  ...
  OwnerDeposit,
  UpdateDepositData,
} from "../services/ownerDepositsService";
...
const OwnerDeposits = () => {
  ...
  const [selectedDeposit, setSelectedDeposit] = useState<OwnerDeposit | null>(
    null,
  );
  ...
  const handleActionConfirm = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Грешка при обновяване на депозита",
      );
    } finally {
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            Депозити за моите автомобили
          </h1>
          <p className="text-gray-600">
            Преглед и управление на всички депозити за вашите автомобили
          </p>
        </div>
        ...
        {deposits.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-8 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Все още няма депозити за вашите автомобили.
            </p>
          </div>
        ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-gray-500">
                                Клиент: {deposit.userId.name} (
                                {deposit.userId.email})
                              </p>
                              <p className="text-sm text-gray-500">
                                Сума: {deposit.amount.toLocaleString("bg-BG")}{" "}
                                лв.
                              </p>
                              <p className="text-sm text-gray-500">
                        ...
                        <div className="flex items-center space-x-4">
                          <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(deposit.status)}`}
                          >
                          ...
                          {deposit.status === "pending" && (
                            <div className="flex space-x-2">
                              ...
                              >
                                {updatingDeposit === deposit._id
                                  ? "Обновяване..."
                                  : "Одобри"}
                              </button>
                              <button
                                onClick={() =>
                                  handleActionClick(deposit, "reject")
                                }
                                disabled={updatingDeposit === deposit._id}
            ...
            <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
              <div className="mt-3">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  {actionType === "approve"
                    ? "Одобрявате депозит за автомобил " +
                      selectedDeposit.listingId.brand +
                      " " +
                      selectedDeposit.listingId.carModel
                    : "Отхвърляте депозит за автомобил " +
                      selectedDeposit.listingId.brand +
                      " " +
                      selectedDeposit.listingId.carModel}
                </p>
                ...
                <div className="flex justify-end space-x-3">
                  ...
                  >
                    {updatingDeposit === selectedDeposit._id
                      ? "Обновяване..."
                      : actionType === "approve"
                        ? "Одобри"
                        : "Отхвърли"}
