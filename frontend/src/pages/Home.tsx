...
import { fetchCars, Car, fetchCarsUnauthenticated } from '../services/carService';
import { checkDepositStatus, DepositStatusResponse } from '../services/depositsService';
import { FaCar, FaSearch, FaMoneyCheckAlt, FaUserCheck, FaQuoteLeft, FaStar, FaMoneyBillWave, FaClock, FaCheckCircle, FaTimesCircle } from 'react-icons/fa';
        ...
        <section className="w-full py-20 bg-gradient-to-br from-yellow-50 to-orange-50">
          <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl font-extrabold text-primary-700 mb-4">Моите депозити</h2>
              <p className="text-lg text-primary-600">Следете статуса на вашите депозити и направете нови</p>
            </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                <div className="bg-white rounded-2xl shadow-lg p-8 max-w-md mx-auto">
                  ...
                  <h3 className="text-xl font-semibold text-gray-700 mb-2">Нямате депозити</h3>
                  <p className="text-gray-600 mb-6">Направете първия си депозит, за да запазите автомобил</p>
                  <Link 
                    to="/cars" 
              ...
              <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                {carsWithDeposits.map(car => {
                  ...
                  return (
                    <div key={car._id} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all border border-primary-100">
                      <div className="relative h-48 overflow-hidden">
                        ...
                        <div className="absolute top-4 right-4">
                          <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium shadow ${getStatusColor(deposit?.status || 'pending')}`}>
                            {getStatusIcon(deposit?.status || 'pending')}
                            <span className="ml-1">{getStatusText(deposit?.status || 'pending')}</span>
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="p-6">
                        ...
                        <p className="text-gray-600 mb-3">{car.year} • {car.fuelType} • {car.transmission}</p>
                        
                        <div className="mb-4">
                          <div className="flex justify-between items-center mb-2">
                            <span className="text-sm text-gray-500">Цена на автомобила:</span>
                            <span className="font-semibold text-primary-600">${car.price.toLocaleString()}</span>
                          </div>
                          {deposit && (
                            <div className="flex justify-between items-center">
                              <span className="text-sm text-gray-500">Вашият депозит:</span>
                        ...
                        <div className="space-y-3">
                          ...
                          {(deposit?.status === 'rejected' || deposit?.status === 'refunded') && (
                            <button
                          ...
                          <div className="mt-4 pt-4 border-t border-gray-200">
                            <div className="text-sm text-gray-600">
                              <div className="flex justify-between">
                                ...
                                <span>{new Date(deposit.createdAt).toLocaleDateString('bg')}</span>
                              </div>
                              {deposit.notes && (
                                <div className="mt-2">
                                  <span className="text-gray-500">Бележки:</span>
                                  <p className="text-gray-700 italic">"{deposit.notes}"</p>
                                </div>
                              )}
                            </div>
                          </div>
