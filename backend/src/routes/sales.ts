...
router.post(
  ...
  [
    ...
    body("saleType")
      .isIn(["full", "leasing"])
      .withMessage("Sale type must be full or leasing"),
    body("totalAmount")
      .isFloat({ min: 0 })
    ...
    body("leaseTerm")
      .optional()
      .isInt({ min: 1, max: 120 })
      .withMessage("Lease term must be between 1 and 120 months"),
    body("interestRate")
      .optional()
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (car.status !== "available" && car.status !== "reserved") {
        return res
          .status(400)
          .json({ message: "Car is not available for sale" });
      }
      ...
      if (saleType === "leasing") {
        if (!downPayment || !monthlyPayment || !leaseTerm) {
          return res.status(400).json({
            message:
              "Down payment, monthly payment, and lease term are required for leasing",
          });
        }
      }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (
        sale.sellerId.toString() !== user._id.toString() &&
        user.role !== "admin"
      ) {
        return res.status(403).json({ message: "Access denied" });
      }
      ...
      if (existingInvoice) {
        return res
          .status(400)
          .json({ message: "Invoice already exists for this sale" });
      }
